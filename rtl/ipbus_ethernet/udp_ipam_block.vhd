...
-- Options
	...
	-- End x"FF"
dhcp_block:  process(mac_clk, rst_macclk)
...
delay_ipam_mode: process(mac_clk, rst_macclk)
...
second_count: process(mac_clk, rst_macclk)
...
-- THA(6)
-- TPA(4)
rarp_block:  process(mac_clk, rst_macclk)
  variable pkt_data: std_logic_vector(7 downto 0);
...
addr_block:  process(mac_clk, rst_macclk)
...
tick_counter:  process(mac_clk, rst_macclk)
...
random: process(mac_clk, rst_macclk)
...
ipam_req_block: process(mac_clk, rst_macclk)
