...
rx_event:  process(mac_clk, rst_macclk)
...
send_data:  process(mac_clk, rst_macclk)
...
state_machine:  process(mac_clk, rst_macclk)
...
udp_event:  process(mac_clk, rst_macclk)
...
udp_short_block: process(mac_clk, rst_macclk)
...
udp_send_data:  process(mac_clk, rst_macclk)
...
do_udp_counter: process(mac_clk, rst_macclk)
...
udp_control_build:  process(mac_clk, rst_macclk)
...
udp_build_data:  process(mac_clk, rst_macclk)
...
do_ipbus_hdr: process(mac_clk, rst_macclk)
...
-- pragma translate_on
      ...
      if udpram_active = '1' and low_addr = '1' and mac_tx_ready_sig = '1' then
        case to_integer(unsigned(addr_sig(5 downto 0))) is
	  when 50 =>
